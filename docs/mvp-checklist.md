...
| 3 | Listagem + busca + cache Redis | ✅ | **Busca/filtros**: `GET /public/venues` aceita `city`, `state`, `min_capacity`, `max_price`, `min_price`, `q`, `amenities` e **disponibilidade por datas** (`start`/`end` → exclui reservas não-canceladas sobrepostas), tudo AND. **Barra unificada**: Localização (1 input que casa cidade/estado/bairro via `loc`), Data (**DateRangePicker** calendário popover) e Valor (**RangeSlider** de duas alças). (ElasticSlider portado segue no repo, sem uso na barra.) **Cache Redis**: listagem sem filtros via cache-aside (TTL 5min) + invalidação; buscas filtradas vão ao Postgres. |
